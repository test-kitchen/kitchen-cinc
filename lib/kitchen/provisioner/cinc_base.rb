...
module Kitchen
  module Provisioner
    ...
    class CincBase < Base
      ...
      # If set to true (which is the default from `cinc generate`), try to update
      ...
      #
      # A Policyfile wins over a Berksfile; with neither present no resolver is
      # loaded and the cookbooks are staged by copying.
      #
      # @return [void]
      # @raise [Kitchen::UserError] if the selected resolver's library cannot
      #   be loaded
      # @see Kitchen::Provisioner::Base#load_needed_dependencies!
      ...
      #
      # @param command [String] the installer script generated by
      #   Mixlib::Install, which is written to disk and then executed
      # @return [String] platform-appropriate shell code
      ...
      # Validates the Policyfile and Berksfile settings before the sandbox is
      # populated.
      #
      # @return [void]
      # @raise [Kitchen::UserError] if a configured Policyfile or Berksfile is
      #   missing, or if a Policyfile was found but this provisioner does not
      #   support Policyfiles
      # @api private
      ...
      #
      # Also writes the idempotency-check config when +enforce_idempotency+ is
      # set.
      #
      # @return [void]
      # @api private
      ...
      #
      # @param data [Hash] the same key/value configuration written to the
      #   regular config file, extended with the failure handler
      # @return [void]
      # @api private
      ...
      # @param _config_filename [String] basename of the config file the client
      #   should read out of +root_path+
      # @return [Array<String>] an array of command line arguments
      # @raise [RuntimeError] always, until a subclass overrides it
      # @api private
      ...
      # Gives the command used to run cinc, joining the individual converge
      # commands with a shell-appropriate separator that stops on failure.
      #
      # @param base_cmd [String] the client invocation the arguments are
      #   appended to, for example `sudo -E /opt/cinc/bin/cinc-client`
      # @return [String] a single shell command
      # @api private
      ...
      # Gives an array of commands, one per converge requested by
      # +multiple_converge+. Only the final one propagates its exit code, and
      # it reads the no-updated-resources config when +enforce_idempotency+ is
      # set.
      #
      # @param base_cmd [String] the client invocation the arguments are
      #   appended to
      # @return [Array<String>] one wrapped shell command per converge
      # @api private
      ...
      #
      # @param base_cmd [String] the client invocation to append arguments to
      # @param configfile [String] basename of the config file to run with
      # @param append [String, nil] extra shell code appended after the
      #   arguments, such as the PowerShell exit-code fragment
      # @return [String] the wrapped, prefixed shell command
      # @api private
