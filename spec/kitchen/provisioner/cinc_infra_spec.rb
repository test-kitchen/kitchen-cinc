...
describe Kitchen::Provisioner::CincInfra do
  ...
  let(:provisioner) do
    Kitchen::Provisioner::CincInfra.new(config).finalize_config!(instance)
  end
  ...
  describe "#run_command" do
    ...
    def self.common_shell_specs
      it "sets config flag on cinc-client" do
      ...
      it "sets log level flag on cinc-client to auto by default" do
      ...
      it "sets force formatter flag on cinc-client" do
        _(cmd).must_match regexify(" --force-formatter", :partial_line)
      end

      it "sets no color flag on cinc-client" do
        _(cmd).must_match regexify(" --no-color", :partial_line)
      end

      it "sets json attributes flag on cinc-client" do
        _(cmd).must_match regexify(
    ...
    describe "for modern Cinc versions" do
      ...
      def self.common_modern_shell_specs
        it "sets local mode flag on cinc-client" do
          _(cmd).must_match regexify(" --local-mode", :partial_line)
        end

        it "sets chef zero port flag on cinc-client" do
